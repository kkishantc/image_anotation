...
function addZoneBox(_objZone) {
  ...
  theDiv.style.top =
    _objZone.y + (document.all ? document.body.scrollTop : pageYOffset);
  theDiv.style.left =
    _objZone.x + (document.all ? document.body.scrollLeft : pageXOffset);
...
function getNextZoneSequence() {
  ...
  getZones().forEach((objZone) => {
    if (objZone.sequence > intHighest) intHighest = objZone.sequence;
  });
...
function getZones() {
  ...
  strZones.split(/\r?\n/).forEach((strLine) => {
    if (strLine.trim().length > 0) {
      ...
      arrZones.push({
        x: parseInt(arrTokens[0] / dblRatio) + getElementPosition(elImage).x,
        y: parseInt(arrTokens[1] / dblRatio) + getElementPosition(elImage).y,
